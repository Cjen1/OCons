...
let ballot_equality_test2 test_ctx =
  assert_bool "Foo"
    (equal (Ballot.bottom ()) (Ballot.Number (0, create_id ())))
...
let ballot_equality_tests =
  [ ("..." >:: fun ctx -> equal Bottom Bottom |> assert_bool "Foo")
  ; ( "..."
    >:: fun ctx ->
  ...
  ; ( "..."
    >:: fun ctx ->
    equal (Number (0, create_id ())) (Number (0, create_id ()))
    |> not |> assert_bool "Foo" )
  ; ( "Different number, different id"
    >:: fun ctx ->
    equal (Number (0, create_id ())) (Number (1, create_id ()))
    |> not |> assert_bool "Foo" ) ]
...
let two_ordered_ids () =
  ...
  if Core.Uuid.compare id1 id2 < 0 then (id1, id2) else (id2, id1)
...
let ballot_ordering_tests =
  ...
  ; ( "Check lexicographical ordering of ballots with the same id"
    ...
    && less_than (Number (1, id)) (Number (0, id)) |> not )
    |> assert_bool "Foo" )
  ...
  ; ( "Check that comparisons with bottom yield expected integer result"
    >:: fun ctx -> Ballot.compare Bottom Bottom |> assert_equal 0 )
  ; ( "Check that comparisons with bottom yield expected integer result"
    >:: fun ctx ->
    let id = create_id () in
  ...
  ; ( "Check comparison is made first based on integer part of ballot"
    ...
    && Ballot.compare (Number (0, id2)) (Number (1, id1)) < 0 )
    |> assert_bool "Foo" )
  ; ( "Check comparison is made first based on integer part of ballot"
    >:: fun ctx ->
    let id1, id2 = (create_id (), create_id ()) in
    ( Ballot.compare (Number (1, id1)) (Number (0, id2)) > 0
    && Ballot.compare (Number (1, id2)) (Number (0, id1)) > 0 )
    |> assert_bool "Foo" )
  ; ( "Check comparison is made first based on integer part of ballot"
    >:: fun ctx ->
    let id = create_id () in
...
let ballot_test_suite =
  "Ballot test suite" >::: concat [ballot_equality_tests; ballot_ordering_tests]
