...
type t =
  ...
  ; (* The state of the application it is replicating *)
    mutable app_state: Types.app_state
  ; (* Temporary state of application type *)
...
let list_of_cmds cmds =
  Lwt.return
    (Queue.iter
       ~f:(fun cmd ->
         Lwt.ignore_result
           (Lwt_io.print (" " ^ Types.string_of_command cmd ^ "\n")))
       cmds)
...
let list_of_proposals proposals =
  Lwt.return
    (List.iter proposals ~f:(fun p ->
         Lwt.ignore_result
           (Lwt_io.print (" " ^ Types.string_of_proposal p ^ "\n"))))

let print_replica replica =
  Lwt_io.printl ("Replica id " ^ Uuid.to_string replica.id)
  >>= fun () ->
  Lwt_io.printl
    ( "Slot-in: "
    ^ string_of_int replica.slot_in
    ^ " | Slot-out: "
    ^ string_of_int replica.slot_out )
  >>= fun () ->
  Lwt_io.printl "Requests: ["
  >>= fun () ->
  list_of_cmds replica.requests
  >>= fun () ->
  Lwt_io.printl "]"
  >>= fun () ->
  Lwt_io.printl "Proposals: ["
  >>= fun () ->
  list_of_proposals replica.proposals
  >>= fun () ->
  Lwt_io.printl "]"
  >>= fun () ->
  Lwt_io.printl "Decisions: ["
  >>= fun () ->
  list_of_proposals replica.decisions >>= fun () -> Lwt_io.printl "]"
...
let perform replica c =
  ...
  let is_lower_slot =
    match
      List.Assoc.find decisions_inv
        ~equal:(fun c1 c2 -> Types.commands_equal c1 c2)
        c
    with
    | None ->
        false
  ...
  if is_lower_slot || isreconfig op then (
    ...
    if replica.slot_out mod 10 = 0 then
      Lwt_list.iter_p
        (fun uri ->
          let msg =
            Message.SlotOutUpdateMessage (replica.slot_out, replica.id)
          in
          Message.send_request msg uri)
        (List.concat [replica.leaders; replica.acceptors])
      |> Lwt.ignore_result )
  else
    (* ATOMIC - May need to make this an atomic execution block *)
    (* Update application state *)
    Lwt.ignore_result (write_to_log TRACE (Types.string_of_command c)) ;
  ...
  Lwt.ignore_result
    (write_with_timestamp INFO
       ("Update application state to " ^ Types.string_of_state next_state)) ;
  replica.slot_out <- replica.slot_out + 1 ;
  (* END ATOMIC *)
  Lwt.ignore_result
    (write_with_timestamp INFO
       ( "Send response message (" ^ string_of_int cid ^ ", "
       ^ Types.string_of_result results
       ^ ") to " ^ Uri.to_string uri )) ;
...
let queue_mem (q : 'a Queue.t) (x : 'a) (equal : 'a -> 'a -> bool) =
  let b = ref false in
  fst (!b, Queue.iter ~f:(fun y -> b := !b || equal x y) q)

let rec try_execute (replica : t) (p : proposal) =
  let s, c = p in
  if queue_mem replica.requests c Types.commands_equal && replica.slot_in = s
  then
    Lwt.ignore_result
      (write_with_timestamp INFO "this command is already in the requests")
  else
    Lwt.ignore_result
      (write_with_timestamp INFO "this command is not already in the requests") ;
  ...
  | Some c' ->
      ...
      | Some c'' ->
          ...
          let new_proposals =
            List.filter replica.proposals ~f:(fun p' ->
      ...
      perform replica c' ; try_execute replica p
...
let receive_decision (replica : t) (p : proposal) : unit =
  Lwt.ignore_result
    (write_with_timestamp INFO
       ( "Receive decision for " ^ Types.string_of_proposal p ^ ", slot_out = "
       ^ string_of_int replica.slot_out
       ^ ", slot_in = "
       ^ string_of_int replica.slot_in )) ;
...
let start_server (replica : t) (host : string) (port : int) =
  Message.start_new_server ~request_callback:(receive_request replica)
    ~proposal_callback:(receive_decision replica) host port

let queue_filter (q : 'a Queue.t) (p : 'a -> bool) =
  let q' = Queue.copy q in
  Queue.clear q ;
  Queue.iter ~f:(fun x -> if p x then Queue.enqueue q x else ()) q'

let do_proposal (c : Types.command) (replica : t) =
  Lwt.ignore_result
    (write_with_timestamp INFO
       ( "There is no command yet committed to slot "
       ^ string_of_int replica.slot_in )) ;
  (*
...
*)
  ...
  Lwt.ignore_result
    (write_with_timestamp INFO
       ("Propose " ^ Types.string_of_proposal (replica.slot_in, c))) ;
...
let propose replica =
  ...
  if slot_in < slot_out + window && not (Queue.is_empty replica.requests) then (
    (* Select a request *)
    ...
    Lwt.ignore_result
      (write_with_timestamp INFO
         ("Increment slot_in to " ^ string_of_int replica.slot_in)) ;
    Lwt.return_unit )
...
let new_replica host port replica_uris leader_uris acceptor_uris =
  ...
  Lwt.join
    [ (* Start the server on the specified (host,port) pair.
      ...
      >>= fun uri -> print_uri uri >>= fun () -> fst @@ Lwt.wait () )
