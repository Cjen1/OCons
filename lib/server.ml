...
module Make_Server (S : Server) : sig
  ...
  let string_of_sockaddr s = match s with
    ...
    | Lwt_unix.ADDR_INET (inet,p) -> Unix.string_of_inet_addr inet ^ ":" ^ Int.to_string p

  let accept_connection (fd, sockaddr) =
    ...
    let callback =
      ...
      let* () = Logs_lwt.debug (fun m -> m "Connection initialised from %s" (string_of_sockaddr sockaddr ) ) in
      let* () = S.connected_callback (ic, oc) in
  ...
  let create_server sock =
    let rec serve () =
      ...
      Lwt.join [accept_connection conn; serve ()]
    in
    serve
