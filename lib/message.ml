...
let serialize_phase1_response acceptor_id ballot_num accepted sl_gc :
    Yojson.Basic.json =
  let acceptor_id =
    ("acceptor_id", `String (Types.string_of_id acceptor_id))
  in
  let ballot_json = Yojson.Basic.Util.to_assoc (Ballot.serialize ballot_num) in
  let pvalues_json =
    Yojson.Basic.Util.to_assoc
      (`Assoc [("pvalues", Pval.serialize_list accepted)])
  in
  let sl_gc_json = ("sl_gc", `Int sl_gc) in
  let response_json =
    `Assoc
      ( acceptor_id :: sl_gc_json
      :: Core.List.concat [ballot_json; pvalues_json] )
  in
...
let deserialize_phase1_response (response_json : Yojson.Basic.json) =
  ...
  , Yojson.Basic.Util.to_int sl_gc )
...
let local ?(request_callback : (command -> unit) option)
    ...
    ?(phase1_callback :
       (   Ballot.t
        -> Types.unique_id * Ballot.t * Pval.t list * Types.slot_number)
       option)
  ...
  @@ object
       ...
       method slot_out_update_impl params release_param_caps =
         ...
         release_param_caps () ;
         match slot_out_update_callback with
         | Some f ->
             f (slot_out, rep_id) ;
       ...
       method phase2_impl params release_param_caps =
         ...
         let pvalue =
           Params.pvalue_get params |> Yojson.Basic.from_string
           |> Pval.deserialize
         in
         release_param_caps () ;
         match phase2_callback with
         | Some f ->
       ...
       method phase1_impl params release_param_caps =
         ...
         let ballot_number =
           Params.ballot_number_get params
           |> Yojson.Basic.from_string |> Ballot.deserialize
         in
         release_param_caps () ;
         match phase1_callback with
         | Some f ->
             let acceptor_id, ballot_num', pvalues, sl_gc = f ballot_number in
             let json =
               serialize_phase1_response acceptor_id ballot_num' pvalues sl_gc
             in
             let result_str = Yojson.Basic.to_string json in
             let response, results =
               Service.Response.create Results.init_pointer
       ...
       method client_response_impl params release_param_caps =
         ...
         let result =
           match Result.get result_reader with
           | Result.Failure ->
               Types.Failure
           | Result.Success ->
               Types.Success
       ...
       method send_proposal_impl params release_param_caps =
         ...
         let operation =
           match Command.Operation.get op_reader with
           ...
           | Command.Operation.Remove r_struct ->
               ...
               Types.Remove k
           | Command.Operation.Undefined _ ->
               raise Undefined_oper
         in
         (* Form the proposal from the message parameters *)
         let proposal =
           ( slot_number
           , ((Uuid.of_string id, Uri.of_string uri), command_id, operation) )
         in
         (* Do something with the proposal here *)
         (* This is nonsense at the moment *)
         (match proposal_callback with None -> () | Some g -> g proposal) ;
         (* Release capabilities, doesn't matter for us *)
         release_param_caps () ;
         (* Return an empty response *)
         Service.return_empty ()
...
let client_request_rpc t (cmd : Types.command) =
  ...
  >|= function Ok () -> () | Error e -> Hashtbl.clear sturdy_refs
...
type message =
  | ClientRequestMessage of command
...
let start_new_server ?request_callback ?proposal_callback ?response_callback
    ?phase1_callback ?phase2_callback ?slot_out_update_callback host port =
  let listen_address = `TCP (host, port) in
  let config =
    Capnp_rpc_unix.Vat_config.create ~serve_tls:false ~secret_key:`Ephemeral
      listen_address
  in
  (* let service_id = Capnp_rpc_unix.Vat_config.derived_id config "main" in *)
  let service_id =
    Capnp_rpc_lwt.Restorer.Id.derived ~secret:"" (host ^ string_of_int port)
  in
  let restore =
    Capnp_rpc_lwt.Restorer.single service_id
      (local ?request_callback ?proposal_callback ?response_callback
         ?phase1_callback ?phase2_callback ())
  in
  Capnp_rpc_unix.serve config ~restore
  >|= fun vat -> Capnp_rpc_unix.Vat.sturdy_uri vat service_id
...
let phase1_rpc t (b : Ballot.t) =
  ...
  let request, params = Capability.Request.create Params.init_pointer in
  let open Api.Builder.Message in
  Params.ballot_number_set params
    (b |> Ballot.serialize |> Yojson.Basic.to_string) ;
  Capability.call_for_value_exn t method_id request
  >|= Results.result_get >|= Yojson.Basic.from_string
  >|= deserialize_phase1_response

(* TODO: Pattern matching here exhaustive *)
let send_phase1_message (b : Ballot.t) uri =
  service_from_uri uri >>= function Some service -> phase1_rpc service b
...
let send_phase2_message (pval : Pval.t) uri =
  service_from_uri uri >>= function Some service -> phase2_rpc service pval
