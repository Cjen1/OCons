...
type measurement = Empty | Request of Core.Time.t | Responded
...
let bandwidth_of_times t0 t1 n =
  string_of_float
    (float_of_int n /. Core.Time.Span.to_sec (Core.Time.abs_diff t0 t1))
...
let result_callback (response : Types.command_id * Types.result) =
  ...
  Lwt_main.run
    ...
    | Empty ->
        failwith "We shouldn't reach here"
    | Request t ->
        let%lwt () =
          write_to_log WARN ("Receive first response " ^ string_of_int cid)
        in
...
let new_client host port replica_uris =
  Message.start_new_server ~response_callback:result_callback host port
  >>= fun uri -> Lwt.return (initialize uri replica_uris)
...
let send_request_message client operation =
  ...
  Lwt_list.iter_p
    (fun uri ->
      ...
      Message.send_request message uri)
    client.replica_uri_list
...
let send_timed_request client cid =
  if cid = 1 then t0 := Core.Time.now () ;
