...
let rec read_as_string file_str chan =
  Lwt_io.read_line_opt chan
  >>= function
  | None ->
      Lwt.return file_str
  | Some line ->
      read_as_string (file_str ^ line ^ "\n") chan

(* Open a file from a given path and read into a string Lwt.t *)
let read_string_lwt (path : Lwt_io.file_name) : string Lwt.t =
  Lwt_io.open_file Lwt_io.Input path >>= fun chan -> read_as_string "" chan
  ...
  let get_host_port_pairs role_str =
    List.map
      (fun json ->
        ...
        (host, port))
      (Util.to_list (Util.member role_str config_json))
  in
