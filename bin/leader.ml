...
(* Sample leader code *)
let run_leader host port replica_uris acceptor_uris log_dir =
  let log_directory = log_dir ^ "op_leader-" ^ host ^ "-" ^ (string_of_int port) in
  Lwt_main.run (
...
let command =
  Command.basic
    ~summary:"Leader for Ocaml Paxos"
    Command.Let_syntax.(
      let%map_open
        ...
        in fun () -> 
            ...
            in run_leader "127.0.0.1" 2380 replica_uris acceptor_uris log_dir
    )
