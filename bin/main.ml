...
let run_client' host port uris =
  ...
  Lwt_main.run
    ...
    >>= fun () ->
    Lwt_io.printl "Spinning up a client"
    >>= fun () ->
    Client.new_client host port uris
    >>= fun client ->
    let rec commands n =
      match n with
      | 0 ->
      ...
      | n ->
          ...
          let rand_cmd =
            ...
            | 0 ->
                Nop
            | 1 ->
                Create (Random.int 10, string_of_int (Random.int 10))
            | 2 ->
                Read (Random.int 10)
            | 3 ->
                Update (Random.int 10, string_of_int (Random.int 10))
            | _ ->
                Remove (Random.int 10)
          in
          Client.send_request_message client rand_cmd
          >>= fun () -> commands (n - 1)
    in
    commands 5 >>= fun () -> fst @@ Lwt.wait () )
  ...
  Lwt_main.run
    ...
    let rec commands m =
      ...
      | 0 ->
          Lwt.return_unit
      | m' ->
          Client.send_timed_request client (n + 1 - m')
          >>= fun () -> Lwt_unix.sleep 0.5 >>= fun () -> commands (m' - 1)
    in
    commands n >>= fun () -> waiter )
...
let run_client host port config =
  let replica_uris =
    List.map config.replica_addrs ~f:(fun (host, port) ->
        Message.uri_from_address host port)
  in
...
let run_replica host port config =
  ...
  let leader_uris =
    List.map config.leader_addrs ~f:(fun (host, port) ->
        Message.uri_from_address host port)
  in
...
let run_leader host port config =
  ...
  in
  let acceptor_uris =
    List.map config.acceptor_addrs ~f:(fun (host, port) ->
        Message.uri_from_address host port)
  in
...
(* Run this application as an acceptor, serving over the (host,port) address
   under a global configuration given in config *)
let run_acceptor host port _ = run_acceptor' host port
...
let start ?trace node host port config =
  ...
  | "client" -> (
    ...
    | Some n ->
        run_trace_client host port config n )
  | "replica" ->
...
let sanitise_node (node_string : string) : string =
  ...
  | "acceptor" ->
      node_string
  | _ ->
      raise (Invalid_argument "Invalid type of node given")

let sanitise_port (port_string : string) : int =
  try
    let port_int = int_of_string port_string in
    if port_int > 1024 && port_int <= 65535 then port_int
...
let command =
  Command.basic ~summary:"Foo foo foo foo bar"
    Command.Let_syntax.(
      let%map_open some_node_string = flag "--node" (optional string) ~doc:""
      ...
      fun () ->
        ...
        | None ->
            raise (Invalid_argument "No node type supplied")
        | Some node_string -> (
          match some_config_path with
          | None ->
              raise (Invalid_argument "No path to config file supplied")
          | Some config_path -> (
            ...
            | Some host_string, Some port_string -> (
              ...
              | None ->
                  start
                    ...
                    (sanitise_config config_path) )
            | _, _ ->
                raise (Invalid_argument "Host / port not supplied") ) ))
