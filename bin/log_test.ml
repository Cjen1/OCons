...
module P_t_ref = struct
  ...
  let decode buf ~offset = 
    let decode_buf = Bytes.sub buf ~pos:offset ~len:(Bytes.length buf - offset) in
...
let test_ref os log () = 
  let log = List.fold_right ~f:(fun op log -> P_ref.change log op) ~init:log os in
  P_ref.sync log >>= fun _ ->
  Lwt.return_unit

let bench f = 
  let st = Unix.gettimeofday () in
  f () >>= fun _ ->
  Unix.gettimeofday () -. st |> Lwt.return

let test () = 
  let op = P_t_ref.Set (10, {command = {op = StateMachine.Read "asdf"; id=1}; term=1; index=10}) in
  let os = List.init 100000 ~f:(fun _ -> op) in
  let log_path = "log1.tmp" in
  P_ref.of_file log_path >>= fun log -> 
  bench (test_ref os log) >>= fun time ->
  Fmt.pr "Reference: %.3f\n" time;
  Lwt.return_unit
