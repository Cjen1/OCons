...
let%expect_test "transit_leader" =
  ...
  [%expect
    ...
    +Candidate for term 3
    +Leader for term 3
    t': {log: []; commit_index:-1; current_term: 3; node_state:Leader{heartbeat:1; rep_ackd:
...
let%expect_test "Loop" =
  ...
  [%expect
    {|
    t: {log: [{command: Command((Read m1), 1); term : 12}]; commit_index:-1; current_term: 12; node_state:Leader{heartbeat:1; rep_ackd:
  ...
  [%expect
    {|
    t1: {log: [{command: Command((Read m1), 1); term : 12}]; commit_index:-1; current_term: 12; node_state:Follower(5)}
    a1:
  ...
  [%expect
    {|
    t2: {log: [{command: Command((Read m1), 1); term : 12}]; commit_index:-1; current_term: 12; node_state:Follower(5)}
    a2:
    [Send(0,AppendEntriesResponse {term: 12; success: Ok: 0})] |}] ;
  ...
  [%expect
    {|
    t: {log: [{command: Command((Read m1), 1); term : 12}]; commit_index:0; current_term: 12; node_state:Leader{heartbeat:1; rep_ackd:
  ...
  [%expect
    {|
    t: {log: [{command: Command((Read m1), 1); term : 12},{command: Command(
              (Read m2), 2); term : 12}]; commit_index:0; current_term: 12; node_state:Leader{heartbeat:1; rep_ackd:
  ...
  [%expect
    {|
    t: {log: [{command: Command((Read m1), 1); term : 13},{command: Command(
              (Read m4), 3); term : 13}]; commit_index:-1; current_term: 13; node_state:Leader{heartbeat:1; rep_ackd:
  ...
  [%expect
    {|
    t: {log: [{command: Command((Read m1), 1); term : 12},{command: Command(
              (Read m4), 3); term : 13}]; commit_index:-1; current_term: 13; node_state:Follower(5)}
    actions:
    [Send(1,AppendEntriesResponse {term: 13; success: Ok: 1})] |}] ;
  let aerm4 = AppendEntriesResponse {term= 13; success= Ok 1; trace= -1.} in
  ...
  [%expect
    {|
    t: {log: [{command: Command((Read m1), 1); term : 13},{command: Command(
              (Read m4), 3); term : 13},{command: Command((Read m5), 4); term : 13},{command: Command(
              (Read m6), 5); term : 13}]; commit_index:1; current_term: 13; node_state:Leader{heartbeat:1; rep_ackd:
  ...
  [%expect
    ...
    t: {log: [{command: Command((Read m1), 1); term : 12},{command: Command(
              (Read m4), 3); term : 13}]; commit_index:-1; current_term: 14; node_state:Candidate {quorum:{threshold 1, elts:
    []}, timeout:1}}
  ...
  [%expect
    ...
    t: {log: [{command: Command((Read m1), 1); term : 12},{command: Command(
              (Read m2), 2); term : 12}]; commit_index:0; current_term: 14; node_state:Follower(5)}
    actions:
  ...
  [%expect
    ...
    +Leader for term 14
    t: {log: [{command: Command((Read m1), 1); term : 14},{command: Command(
              (Read m4), 3); term : 14}]; commit_index:-1; current_term: 14; node_state:Leader{heartbeat:1; rep_ackd:
    ...
    actions: [Send(0,AppendEntries {term: 14; leader_commit: -1; prev_log_index: -1; prev_log_term: 0; entries_length: 2; entries:
                                                               [{command: Command(
                                                                (Read m1), 1); term : 14},{command: Command(
                                                                (Read m4), 3); term : 14}]})
  ...
  [%expect
    {|
    t: {log: [{command: Command((Read m1), 1); term : 14},{command: Command(
              (Read m4), 3); term : 14},{command: Command((Read m7), 6); term : 14}]; commit_index:-1; current_term: 14; node_state:Leader{heartbeat:1; rep_ackd:
  ...
  [%expect
    ...
    +Follower for term 100
    t0:
    {log: [{command: Command((Read m1), 1); term : 12},{command: Command(
               (Read m2), 2); term : 12}]; commit_index:0; current_term: 100; node_state:Follower(5)}

    t1:
    {log:
    [{command: Command((Read m1), 1); term : 13},{command: Command((Read m4), 3); term : 13},{command: Command(
     (Read m5), 4); term : 13},{command: Command((Read m6), 5); term : 13}]; commit_index:1; current_term: 100; node_state:Follower(5)}

    t2:
    {log:
    [{command: Command((Read m1), 1); term : 14},{command: Command((Read m4), 3); term : 14},{command: Command(
     (Read m7), 6); term : 14}]; commit_index:-1; current_term: 100; node_state:Follower(5)} |}] ;
  let rvr0 term =
    RequestVoteResponse
      { term
      ; start_index= 0
  ...
  [%expect
    ...
    t: {log: [{command: Command((Read m1), 1); term : 12},{command: Command(
              (Read m2), 2); term : 12}]; commit_index:0; current_term: 102; node_state:Candidate {quorum:{threshold 1, elts:
    []}, timeout:1}}
  ...
  [%expect
    ...
    +Leader for term 102
    t: {log: [{command: Command((Read m1), 1); term : 12},{command: Command(
              (Read m4), 3); term : 102},{command: Command((Read m5), 4); term : 102},{command: Command(
              (Read m6), 5); term : 102}]; commit_index:0; current_term: 102; node_state:Leader{heartbeat:1; rep_ackd:
    ...
    actions: [Send(1,AppendEntries {term: 102; leader_commit: 0; prev_log_index: 0; prev_log_term: 12; entries_length: 3; entries:
                                                               [{command: Command(
                                                                (Read m4), 3); term : 102},{command: Command(
                                                                (Read m5), 4); term : 102},{command: Command(
                                                                (Read m6), 5); term : 102}]})
  ...
  [%expect
    ...
    t: {log: [{command: Command((Read m1), 1); term : 13},{command: Command(
              (Read m4), 3); term : 13},{command: Command((Read m5), 4); term : 13},{command: Command(
              (Read m6), 5); term : 13}]; commit_index:1; current_term: 103; node_state:Candidate {quorum:{threshold 1, elts:
    []}, timeout:1}}
  ...
  [%expect
    ...
    +Leader for term 103
    t: {log: [{command: Command((Read m1), 1); term : 13},{command: Command(
              (Read m4), 3); term : 13},{command: Command((Read m7), 6); term : 103},{command: Command(
              (Read m6), 5); term : 103}]; commit_index:1; current_term: 103; node_state:Leader{heartbeat:1; rep_ackd:
    [{0, -1}, {2, -1}]; rep_sent:[{0, 3}, {2, 3}]}
    actions: [Send(0,AppendEntries {term: 103; leader_commit: 1; prev_log_index: 1; prev_log_term: 13; entries_length: 2; entries:
                                                               [{command: Command(
                                                                (Read m7), 6); term : 103},{command: Command(
                                                                (Read m6), 5); term : 103}]})
  ...
  [%expect
    ...
    t: {log: [{command: Command((Read m1), 1); term : 14},{command: Command(
              (Read m4), 3); term : 14},{command: Command((Read m7), 6); term : 14}]; commit_index:-1; current_term: 101; node_state:Candidate {quorum:{threshold 1, elts:
    []}, timeout:1}}
  ...
  [%expect
    ...
    +Received vote from 0
    +Leader for term 101
    t: {log: [{command: Command((Read m1), 1); term : 101},{command: Command(
              (Read m4), 3); term : 101},{command: Command((Read m7), 6); term : 101}]; commit_index:-1; current_term: 101; node_state:Leader{heartbeat:1; rep_ackd:
    [{0, -1}, {1, -1}]; rep_sent:[{0, 2}, {1, 2}]}
    actions: [Send(0,AppendEntries {term: 101; leader_commit: -1; prev_log_index: -1; prev_log_term: 0; entries_length: 3; entries:
                                                               [{command: Command(
                                                                (Read m1), 1); term : 101},{command: Command(
                                                                (Read m4), 3); term : 101},{command: Command(
                                                                (Read m7), 6); term : 101}]})
...
let%expect_test "Missing elements" =
  ...
  [%expect
    {|
    t: {log: [{command: Command((Read m1), 1); term : 12},{command: Command(
              (Read m2), 2); term : 12}]; commit_index:-1; current_term: 12; node_state:Leader{heartbeat:1; rep_ackd:
    [{1, -1}, {2, -1}]; rep_sent:[{1, 1}, {2, 1}]}
  ...
  let aer = AppendEntriesResponse {term= 12; success= Error (-1); trace= -1.} in
  ...
  [%expect
    ...
    actions: [Send(1,AppendEntries {term: 12; leader_commit: -1; prev_log_index: -1; prev_log_term: 0; entries_length: 2; entries:
                                                               [{command: Command(
                                                                (Read m1), 1); term : 12},{command: Command(
                                                                (Read m2), 2); term : 12}]})] |}]
