...
struct
  ...
  (* Use a pseudoleader for efficiency *)
  (* So only that node can apply requests *)
  (* As soon as we believe that that leader has failed we failover to the regular conspire stuff *)

  let get_current_leader t =
    let leader_id = t.rep.state.term % t.config.replica_count in
    List.nth_exn t.config.replica_ids leader_id

  let can_apply_requests t =
    let current = get_current_leader t in
    current = t.config.node_id || not (is_live t current)

  let available_space_for_commands t = if can_apply_requests t then 65536 else 0

  let replication ?dst ?(force = false) (t : t) =
    let send_ctree t dst =
    ...
    | Some dst ->
        ...
        Option.iter (R.get_cons_update ~force t.rep) ~f:(fun up ->
            Act.send dst (ConsUpdate up) )
    | None ->
        List.iter t.config.other_replica_ids ~f:(send_ctree t) ;
        Option.iter (R.get_cons_update ~force t.rep) ~f:(fun up ->
  ...
  let check_conflict_recovery t =
    ...
    if max_term > t.rep.state.vterm then
      ...
      if num_max_term_votes >= t.config.quorum_size then (
        ...
        t.rep.change_flag <- true ;
        if get_current_leader t = t.config.node_id then
          traceln "Optimistic Serialiser for term %d" max_term ;
        if can_apply_requests t then
        ...
        else Queue.clear t.command_queue )
