...
let%expect_test "Loop" =
  ...
  [%expect
    {|
    t: {log: [{command: Command(Read m1, 1); term : 12},{command: Command(Read m4, 3); term : 13}]; commit_index:-1; current_term: 13; node_state:Follower{timeout:0; voted_for:1}}
    actions:
  ...
  [%expect
    ...
    t1:
    {log:
    [{command: Command(Read m1, 1); term : 13},{command: Command(Read m4, 3); term : 13},{command: Command(Read m5, 4); term : 13},{command: Command(Read m6, 5); term : 13}]; commit_index:1; current_term: 100; node_state:Follower{timeout:0; voted_for:None}}

    t2:
    {log:
    [{command: Command(Read m1, 1); term : 14},{command: Command(Read m4, 3); term : 14},{command: Command(Read m7, 6); term : 14}]; commit_index:-1; current_term: 100; node_state:Follower{timeout:0; voted_for:None}} |}] ;
  let rvr0 term =
    RequestVoteResponse
      { term
      ; start_index= 0
